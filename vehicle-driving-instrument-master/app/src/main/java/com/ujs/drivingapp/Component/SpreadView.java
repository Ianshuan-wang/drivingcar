 ...
 * @create time 2/6/2022 11:41 AM.
 * @description 水波扩散view$
 */
public class SpreadView extends View {
    ...
    public SpreadView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        radius = a.getInt(R.styleable.SpreadView_spread_center_radius, radius);
        maxRadius = a.getInt(R.styleable.SpreadView_spread_max_radius, maxRadius);
        int centerColor = a.getColor(R.styleable.SpreadView_spread_center_color, ContextCompat.getColor(context, R.color.bg_blue));
        int spreadColor = a.getColor(R.styleable.SpreadView_spread_spread_color, ContextCompat.getColor(context, R.color.bg_blue));
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        double alphaStep = (double) distance / maxRadius * 255;
        for (int i = 0; i < spreadRadius.size(); i++) {
            ...
            if (alpha > 0 && width < maxRadius) {
                alpha = (int) Math.floor(alpha - alphaStep > 0 ? alpha - alphaStep : 1);
                alphas.set(i, alpha);
                spreadRadius.set(i, width + distance);
            }
        }
        //当最外层扩散圆半径达到最大半径时添加新扩散圆
        if (spreadRadius.get(spreadRadius.size() - 1) >= 200) {
            spreadRadius.add(0);
            alphas.add(255);
        }
